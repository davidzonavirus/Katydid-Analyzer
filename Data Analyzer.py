...
from PyQt5.QtWidgets import (QApplication, QMainWindow, QPushButton, QVBoxLayout, QHBoxLayout, 
                           ...
                           QFormLayout, QDialogButtonBox, QTextBrowser, QSizePolicy)
...
class KatydidAnalyzer2(QMainWindow):
    ...
    def setup_analysis_interface(self):
        ...
        # Create a new central widget - use light background for dark mode readability
        central_widget = QWidget()
        central_widget.setObjectName("analysisCentralWidget")
        central_widget.setStyleSheet("""
            #analysisCentralWidget {
        ...
        # Add header with controls button and exit button
        header_layout = QHBoxLayout()
        
        # Add Show Controls button
        controls_btn = QPushButton("Show Controls")
        controls_btn.setStyleSheet("""
            ...
            QPushButton:hover { background-color: rgba(0, 150, 0, 0.8); }
            QPushButton:pressed { background-color: rgba(0, 100, 0, 0.9); }
        """)
        controls_btn.clicked.connect(self.show_controls_help)
        header_layout.addWidget(controls_btn)
    ...
    def create_period_histogram_tab(self):
        ...
        self.period_info_label.setStyleSheet("color: #1a1a1a; background-color: #e8e8e8; padding: 5px; border-radius: 3px;")
    ...
    def create_ratio_histogram_tab(self):
        ...
        self.ratio_info_label.setStyleSheet("color: #1a1a1a; background-color: #e8e8e8; padding: 5px; border-radius: 3px;")
    ...
    def save_waveform_files(self):
        ...
        # Prompt user for folder name (same as Wav Analyzer workflow)
        folder_name, ok = QInputDialog.getText(
            self, "Save Waveform Files", 
            "Enter folder name for output files:")
        
        if not ok or not folder_name:
            return
        
        # Ask user for directory to save the folder in (default: WAV/CSV file location)
        default_dir = os.path.dirname(self.wav_file_path) if hasattr(self, 'wav_file_path') and self.wav_file_path else (os.path.dirname(self.csv_file_path) if hasattr(self, 'csv_file_path') and self.csv_file_path else os.getcwd())
        parent_dir = QFileDialog.getExistingDirectory(self, "Select Directory to Save Folder", default_dir)
        if not parent_dir:
            return
    ...
    def show_controls_help(self):
        ...
        help_text = """
        <html>
        <h2>Data Analyzer Controls</h2>
        
        <h3>Navigation (Waveform & Histogram tabs):</h3>
        <ul>
        ...
        <h3>Table Tab:</h3>
        <ul>
            <li><b>C:</b> Copy column (copy waveform segments)</li>
            <li><b>=:</b> Save waveform files (CSV + WAV)</li>
        </ul>
        
        <h3>Waveform Tab:</h3>
        <ul>
            <li><b>K:</b> Select mode peak and set period range</li>
            <li><b>L:</b> Manually set x-axis range</li>
        </ul>
        
        <h3>Histogram Tabs:</h3>
        <ul>
        ...
        <h3>General:</h3>
        <ul>
            <li><b>Escape:</b> Exit application</li>
        </ul>
        </html>
        """
        help_dialog = QDialog(self)
        help_dialog.setWindowTitle("Data Analyzer Controls")
        help_dialog.setMinimumSize(550, 450)
        help_dialog.resize(600, 500)
        layout = QVBoxLayout()
        help_browser = QTextBrowser()
        help_browser.setHtml(help_text)
        help_browser.setMinimumSize(400, 350)
        help_browser.setStyleSheet("QTextBrowser { font-size: 14px; padding: 15px; }")
